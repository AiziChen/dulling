...
static int get_interface(char *name) {
    ...
    strncpy(ifr.ifr_name, name, sizeof(ifr.ifr_name));
    if (ioctl(interface, F_SETFL, &ifr)) {
        perror("Cannot get TUN interface");
        exit(1);
    }
    return interface;
...
static int get_tunnel(char *port, char *secret) {
    ...
    do {
        addrlen = sizeof(addr);
        ssize_t n = recvfrom(tunnel, packet, sizeof(packet), 0,
                             (sockaddr *) &addr, &addrlen);
        if (n <= 0) {
            return -1;
        }
        packet[n] = 0;
    } while (packet[0] != 0 || strcmp(secret, &packet[1]) != 0);
...
int main(int argc, char **argv) {
    ...
    while ((tunnel = get_tunnel(argv[2], argv[3])) != -1) {
        ...
        while (true) {
            ...
            ssize_t length = read(interface, packet, sizeof(packet));
            if (length > 0) {
                // encrypt the packet
                xor_cipher(packet, length, argv[3]);
